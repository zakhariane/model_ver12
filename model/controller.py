...
class Controller:
    ...
    def get_reference_ProductionRate_for_each_electrolyser(self, i, delta_t, curve_of_desired_total_current: np.array):
        ...
        #U = self.empirical_controller_without_PR_control(delta_t, curve_of_desired_total_current)
        U = self.RL_controller(i, delta_t, curve_of_desired_total_current)

        #U = self.empirical_controller_with_PR_control(delta_t, curve_of_desired_total_current) # TODO
    ...
    def empirical_controller_without_PR_control(self, delta_t, curve_of_desired_total_current: np.array):
        # curve_of_desired_total_current[i] is a reference total CURRENT at time t + i*delta_t
        ...
        desired_Rate_at_next_Delta_t = curve_of_desired_total_current[0]

        Total_output = 0
        for i in range(len(self.Plant)):
            Total_output += self.Plant[
                                i].getCurrentTarget() * maxOutput_of_electrolyser  # target (set point) current of electrolyser number i
        ...
        while abs(newTotal_output - desired_Rate_at_next_Delta_t) > maxOutput_of_electrolyser / 2:
            # ошибаемся больше чем на половину выработки одного электролизера
            if newTotal_output < desired_Rate_at_next_Delta_t:
    ...
    def empirical_controller_with_PR_control(self, delta_t, curve_of_desired_total_current: np.array): # TODO
        # curve_of_desired_total_current[i] is a reference total CURRENT at time t + i*delta_t
        ...
        for i in range(len(self.Plant)):
            current_Total_output += self.Plant[i].getCurrentTarget()*maxOutput_of_electrolyser # target (set point) current of electrolyser number i
        ...
        number_of_required_new_electrolysers = 0
        new_Total_output = current_Total_output
        while abs(new_Total_output - desired_Rate_at_next_Delta_t) > maxOutput_of_electrolyser/2:
            # ошибаемся больше чем на половину выработки одного электролизера
            if new_Total_output < desired_Rate_at_next_Delta_t:
                number_of_required_new_electrolysers += 1
                new_Total_output += maxOutput_of_electrolyser
            else: # newTotal_output > desired_Rate_at_next_Delta_t
                number_of_required_new_electrolysers -= 1
                new_Total_output -= maxOutput_of_electrolyser
