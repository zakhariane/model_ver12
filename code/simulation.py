...
values = [0, 100, 488, 488, 488, 320, 300, 200, 110, 0, 265, 0, 488, 134, 200, 476, 490, 488, 500, 500, 100, 488,
                  70, 0, 488, 80, 190, 280, 488, 488, 210]
...
def get_PRref_init():
    ...
    return np.random.choice(values, size=24) # 24 = 6 hours * (60 / 15)
...
def Generator_of_desired_PR(i, delta_t, Delta_t, PRref, PRref_noised) -> np.array:
    ...
    if i != 0:
        ...
        PRref = np.append(PRref, [np.random.choice(values, size=1)])
...
def simulate_process(Plant, CTR, simulation_time, delta_t, Delta_t = 15*60):
...
def get_agent_working_result(Agent):
    # spawn electrolyses
    num_of_elecs=5
    ...
    results = simulate_process(Plant, CTR, simulation_time = 60 * 60 * 6, delta_t = 1)
